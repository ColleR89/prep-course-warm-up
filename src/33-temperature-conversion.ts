...
const fahrenheitToCelsius = (inp: number): number => {
  return Math.round((inp - 32) * (5 / 9) * 10) / 10;
};
const celsiusToFahrenheit = (inp: number): number => {
  return Math.round((inp * (5 / 9) + 32) * 10) / 10;
};
